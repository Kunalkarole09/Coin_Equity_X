        ...
        <Stack direction="row" alignItems="center" spacing={1.5} mb={2}>
          <Avatar
            sx={{
              ...
              height: 40,
              background: (theme) =>
                theme.palette.mode === 'dark' ? 'rgba(0, 208, 156, 0.1)' : '#F5F7FA',
              border: '1px solid',
              borderColor: (theme) =>
                theme.palette.mode === 'dark' ? 'rgba(255, 255, 255, 0.1)' : '#E8EAED',
              fontSize: '0.75rem',
              fontWeight: 600,
        ...
        <Stack spacing={1.5}>
          ...
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            ...
            <Typography variant="body2" fontWeight={500} sx={{ fontSize: '0.75rem' }}>
              {quote
                ? `${formatCurrency(convert(quote.l, 'USD', currency, fxRates) || 0, currency, {})} - ${formatCurrency(convert(quote.h, 'USD', currency, fxRates) || 0, currency, {})}`
                : '—'}
            </Typography>
          </Stack>
          ...
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            ...
            <Typography variant="body2" fontWeight={500} sx={{ fontSize: '0.75rem' }}>
              {quote
                ? formatCurrency(convert(quote.o, 'USD', currency, fxRates) || 0, currency, {})
                : '—'}
            </Typography>
          </Stack>
          ...
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            ...
            <Typography variant="body2" fontWeight={500} sx={{ fontSize: '0.75rem' }}>
              {quote
                ? formatCurrency(convert(quote.pc, 'USD', currency, fxRates) || 0, currency, {})
                : '—'}
            </Typography>
          </Stack>
        </Stack>
  ...
  const baseFiltered = useMemo(() => {
    ...
    if (category !== 'all') {
      items = items.filter((s) => {
        ...
        if (category === 'tech')
          return desc.includes('tech') || desc.includes('software') || desc.includes('computer');
        if (category === 'finance')
          return desc.includes('bank') || desc.includes('financial') || desc.includes('insurance');
        if (category === 'healthcare')
          return desc.includes('health') || desc.includes('pharma') || desc.includes('bio');
        return true;
      });
    }
    return items;
          ...
          <Button
            ...
            sx={{
              ...
              '&:hover': {
                ...
                borderColor: (theme) =>
                  theme.palette.mode === 'dark' ? 'rgba(255, 255, 255, 0.2)' : '#00D09C'
              }
            }}
          >
            All
    ...
    <Dialog
      ...
      PaperProps={{
        sx: {
          ...
          border: (theme) =>
            `1px solid ${theme.palette.mode === 'dark' ? 'rgba(139, 92, 246, 0.2)' : 'rgba(109, 40, 217, 0.1)'}`,
          borderRadius: 3
        }
      }}
    >
        ...
        <Stack spacing={3}>
          <Paper
            ...
            sx={{
              ...
              border: (theme) =>
                `1px solid ${theme.palette.mode === 'dark' ? 'rgba(139, 92, 246, 0.15)' : 'rgba(109, 40, 217, 0.08)'}`,
              borderRadius: 2
            }}
          >
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <StatCard
                ...
                value={
                  quote
                    ? formatCurrency(convert(quote.h, 'USD', currency, fxRates) || 0, currency, {})
                    : '—'
                }
                icon={<TrendingUp />}
              />
            ...
            <Grid item xs={12} sm={6}>
              <StatCard
                ...
                value={
                  quote
                    ? formatCurrency(convert(quote.l, 'USD', currency, fxRates) || 0, currency, {})
                    : '—'
                }
                icon={<TrendingDown />}
              />
          ...
          <Box>
            <Typography variant="h6" fontWeight={600} gutterBottom>
              About {stock?.displaySymbol || stock?.symbol}
            </Typography>
...
function StockPriceChart({
  ...
  const chartData = useMemo(() => {
    ...
    const config = {
      '1h': {
        points: 60,
        labels: Array.from({ length: 13 }, (_, i) => (i === 12 ? 'Now' : `${60 - i * 5}m ago`)),
        change,
        primaryWave: 0.2,
        secondaryWave: 0.1
      },
      '24h': {
        points: 48,
        labels: Array.from({ length: 9 }, (_, i) => (i === 8 ? 'Now' : `${24 - i * 3}h ago`)),
        change,
        primaryWave: 0.15,
        secondaryWave: 0.08
      },
      '30d': {
        points: 60,
        labels: Array.from({ length: 7 }, (_, i) => (i === 6 ? 'Now' : `${30 - i * 5}d ago`)),
        change,
        primaryWave: 0.1,
        secondaryWave: 0.05
      },
      '60d': {
        points: 60,
        labels: Array.from({ length: 7 }, (_, i) => (i === 6 ? 'Now' : `${60 - i * 10}d ago`)),
        change,
        primaryWave: 0.08,
        secondaryWave: 0.04
      },
      '90d': {
        points: 60,
        labels: Array.from({ length: 7 }, (_, i) => (i === 6 ? 'Now' : `${90 - i * 15}d ago`)),
        change,
        primaryWave: 0.06,
        secondaryWave: 0.03
      }
      ...
      <Box sx={{ width: '100%', height: 300 }}>
        <ResponsiveContainer>
          <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="time" stroke="#888" style={{ fontSize: '0.75rem' }} tickLine={false} />
            <YAxis
              stroke="#888"
  ...
  return (
    <Paper
      ...
      sx={{
        p: 2,
        background: (theme) =>
          theme.palette.mode === 'dark' ? 'rgba(30, 41, 59, 0.5)' : 'rgba(248, 250, 252, 0.5)',
        border: (theme) => `1px solid ${theme.palette.divider}`,
        height: '100%'
      }}
    >
