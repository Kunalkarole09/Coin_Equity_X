...
export function logPerformanceMetrics() {
  ...
  console.log(
    ...
    currentMetrics.CLS?.toFixed(3) || 'N/A',
    '(Good: < 0.1)'
  );
  console.log(
    'FCP (First Contentful Paint):',
    currentMetrics.FCP?.toFixed(0) || 'N/A',
    'ms (Good: < 1800ms)'
  );
  console.log(
    'LCP (Largest Contentful Paint):',
    currentMetrics.LCP?.toFixed(0) || 'N/A',
    'ms (Good: < 2500ms)'
  );
  console.log(
    'TTFB (Time to First Byte):',
    currentMetrics.TTFB?.toFixed(0) || 'N/A',
    'ms (Good: < 800ms)'
  );
  console.log(
    'INP (Interaction to Next Paint):',
    currentMetrics.INP?.toFixed(0) || 'N/A',
    'ms (Good: < 200ms)'
  );
  console.groupEnd();
...
export function logResourceTiming() {
  ...
  Object.entries(byType).forEach(([type, items]) => {
    ...
    console.log(
      `${type}: ${items.length} items, ${totalDuration.toFixed(0)}ms, ${(totalSize / 1024).toFixed(2)}KB`
    );
  });
  console.groupEnd();
}
