...
const mockCategories = {
  data: [{ id: 7, name: 'Layer 1', num_tokens: 2, market_cap: 1000, volume_24h: 10 }]
};
const mockNews = {
  data: [
    {
      id: 'n1',
      title: 'Test headline',
      description: 'A short news description',
...
const mockQuote = {
  data: {
    1: {
      ...
      name: 'Bitcoin',
      symbol: 'BTC',
      quote: { USD: { price: 10000, percent_change_24h: 2.5 } }
    }
  }
};
...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/api/listings**', (route) =>
    route.fulfill({ status: 200, body: JSON.stringify(mockListings) })
  );
  await page.route('**/api/categories**', (route) =>
    route.fulfill({ status: 200, body: JSON.stringify(mockCategories) })
  );
  await page.route('**/api/news**', (route) =>
    route.fulfill({ status: 200, body: JSON.stringify(mockNews) })
  );
  await page.route('**/api/currency/latest**', (route) =>
    route.fulfill({ status: 200, body: JSON.stringify(mockFx) })
  );
  ...
  await page.route('**/api/info**', (route) =>
    route.fulfill({ status: 200, body: JSON.stringify(mockInfo) })
  );
  await page.route('**/api/quote**', (route) =>
    route.fulfill({ status: 200, body: JSON.stringify(mockQuote) })
  );
});
