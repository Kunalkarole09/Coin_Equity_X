...
function createAppServer(options = {}) {
  ...
  if (!CMC_API_KEY) {
    console.warn(
      'Warning: CMC_API_KEY is not set. Set it in .env or your shell before starting the server.'
    );
  }
  ...
  async function handleApi(req, res) {
    ...
    try {
      switch (requestUrl.pathname) {
        case '/api/news': {
          if (!NEWS_API_KEY)
            return sendJson(res, 500, { error: 'MARKETAUX_API_KEY missing on server' });
        ...
        case '/api/portfolio': {
          if (!mongoConfig.uri) {
            return sendJson(res, 501, {
              error: 'Portfolio storage not configured (MONGODB_URI missing)'
            });
          }
          ...
          if (req.method === 'PUT') {
            ...
            if (!Array.isArray(body))
              return sendJson(res, 400, { error: 'Body must be an array of portfolio entries' });
        ...
        case '/api/chat': {
          ...
          if (!GEMINI_API_KEY)
            return sendJson(res, 500, { error: 'GEMINI_API_KEY missing on server' });
        ...
        case '/api/currency/latest': {
          if (!FX_API_KEY)
            return sendJson(res, 500, { error: 'FREE_CURRENCY_API_KEY missing on server' });
          const qs = requestUrl.searchParams.toString();
        ...
        case '/api/stock/symbols': {
          if (!FINNHUB_API_KEY)
            return sendJson(res, 500, { error: 'FINNHUB_API_KEY missing on server' });
  ...
  return {
    ...
    cost:
      entry.cost === null || entry.cost === undefined || Number.isNaN(Number(entry.cost))
        ...
        : Number(entry.cost),
    costUsd:
      entry.costUsd === null || entry.costUsd === undefined || Number.isNaN(Number(entry.costUsd))
        ? null
