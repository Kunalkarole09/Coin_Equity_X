...
class ViewModel {
    ...
    this.view.renderCoins(
      ...
      helpers,
      {
        onAddPortfolio: async (coin) => {
          await this.model.addPortfolioEntry({
            id: coin.id,
            symbol: coin.symbol,
            name: coin.name,
            quantity: 0,
  ...
  async openDetail(id) {
    ...
    try {
      const [infoPayload, quotePayload] = await Promise.all([
        api.fetchInfo(id),
        api.fetchQuote(id)
      ]);
      const infoErr = infoPayload?.status?.error_code;
      const quoteErr = quotePayload?.status?.error_code;
      if (infoErr && infoErr !== 0)
        throw new Error(`Info error ${infoErr}: ${infoPayload?.status?.error_message || ''}`);
      if (quoteErr && quoteErr !== 0)
        throw new Error(`Quote error ${quoteErr}: ${quotePayload?.status?.error_message || ''}`);
      const coin =
        pickCoinFromPayload(quotePayload, id) || this.model.state.coins.find((c) => c.id === id);
      if (!coin) {
      ...
      this.view.showDetailError(
        `Unable to load details. ${error.message || 'Try refresh; API may be rate limited.'}`
      );
    }
  }
  ...
  async handlePortfolioSubmit() {
    ...
    const entry = {
      ...
      cost,
      costUsd,
      costCurrency: this.model.state.currency
    };
  ...
  getHelpers() {
    ...
    return {
      ...
      formatNumber: (value, digits = 0) =>
        formatNumber(value, state.fxRates, state.currency, digits),
      formatChange,
      sanitizeDescription,
      extractLinks,
      buildSeries,
