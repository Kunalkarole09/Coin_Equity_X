...
export default function Dashboard({
  ...
  const filtered = useMemo(() => {
    ...
    if (category !== 'all') {
      items = items.filter((c) => {
        ...
        if (category === 'layer1')
          return tags.some((t: string) => t.toLowerCase().includes('layer'));
  ...
  const stats = useMemo(() => {
    ...
    const avgChange =
      listings.reduce((sum, c) => sum + (c.quote?.USD?.percent_change_24h || 0), 0) /
      (listings.length || 1);
    ...
    <Box>
      ...
      {isLoading && (
        <LinearProgress
          sx={{
            ...
            borderRadius: 2,
            bgcolor: '#F5F5F5',
            '& .MuiLinearProgress-bar': { bgcolor: '#00D09C' }
          }}
        />
      )}
      ...
      <Box sx={{ mb: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.8125rem' }}>
          Showing {page * itemsPerPage + 1}-{Math.min((page + 1) * itemsPerPage, filtered.length)}{' '}
          of {filtered.length}
        </Typography>
        ...
        >
          ...
          <Typography variant="body2" fontWeight={700} color={lineColor}>
            {payload[0].value >= 1
              ...
              : `${currencySymbol}${payload[0].value.toFixed(6)}`}
          </Typography>
        </Paper>
      );
    }
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Box
        sx={{
          ...
          height: 320,
          background: (theme) =>
            theme.palette.mode === 'dark' ? 'rgba(0, 0, 0, 0.2)' : 'rgba(248, 250, 252, 0.5)',
          borderRadius: 2,
          p: 2
        }}
      >
            ...
            <Stack direction="row" spacing={1} mb={2} flexWrap="wrap" useFlexGap>
              {chips.map((chip) => (
                <Chip
                  ...
                  color={
                    chip.value !== undefined && chip.value !== null && chip.value >= 0
                      ? 'success'
                      : 'error'
                  }
                  variant="filled"
