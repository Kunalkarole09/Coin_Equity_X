...
export async function getListings(): Promise<ApiResponse<Coin[]>> {
  return fetchJson<ApiResponse<Coin[]>>('/api/listings');
}

export async function getCategories(): Promise<ApiResponse<Category[]>> {
  return fetchJson<ApiResponse<Category[]>>('/api/categories?start=1');
}

export async function getNews(): Promise<ApiResponse<NewsItem[]>> {
  return fetchJson<ApiResponse<NewsItem[]>>('/api/news?countries=us&filter_entities=true&limit=10');
}

export async function getFx(): Promise<ApiResponse<Record<string, number>>> {
  return fetchJson<ApiResponse<Record<string, number>>>(
    '/api/currency/latest?base_currency=USD',
    CACHE_TTL_MS,
    true
  );
}
...
export async function getPortfolio(userId?: string): Promise<ApiResponse<PortfolioEntry[]>> {
  ...
  const cacheKey = `/api/portfolio?user=${userId || 'anon'}`;
  return fetchJson<ApiResponse<PortfolioEntry[]>>(
    '/api/portfolio',
    CACHE_TTL_MS,
    true,
    headers,
...
export async function savePortfolio(
  ...
  userId?: string
): Promise<ApiResponse<PortfolioEntry[]>> {
...
export async function getCoinInfo(id: string | number): Promise<CoinInfoResponse> {
  return fetchJson<CoinInfoResponse>(
    `/api/info?id=${encodeURIComponent(String(id))}`,
    CACHE_TTL_MS
  );
}

export async function getCoinQuote(id: string | number): Promise<CoinQuoteResponse> {
  return fetchJson<CoinQuoteResponse>(
    `/api/quote?id=${encodeURIComponent(String(id))}`,
    CACHE_TTL_MS
  );
}
...
export async function sendChatMessage(message: string): Promise<string> {
  ...
  if (!res.ok) {
    const errorMessage =
      data && (data.error || data.detail) ? String(data.error || data.detail) : 'Request failed';
